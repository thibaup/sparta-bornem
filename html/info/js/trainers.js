document.addEventListener('DOMContentLoaded', function() {
  ...
  if (!trainingTimesBody || !contactsContainer || !trainerGroupsContainer) return;
  ...
  fetch(jsonPath)
    ...
    .then(data => {
      ...
      if (Array.isArray(data.times_category_order) && data.trainingTimes) {
        ...
        data.trainingTimes.forEach(item => { if (!trainingTimes.includes(item)) trainingTimes.push(item); });
      } else {
      ...
      if (data.contacts && data.contacts.length > 0) {
        data.contacts.forEach(contact => {
          ...
          if (contact.image) {
            ...
            img.addEventListener('load', () => applyRotation(img, contact.rotate || 'auto'));
      ...
      if (Array.isArray(data.trainer_category_order) && data.trainerGroups) {
        ...
        data.trainerGroups.forEach(g => { if (!trainerGroups.includes(g)) trainerGroups.push(g); });
      } else {
      ...
      if (trainerGroups.length > 0) {
        trainerGroups.forEach(group => {
          ...
          if (group.mainPhoto && group.mainPhoto.src) {
            ...
            mainImg.addEventListener('load', () => applyRotation(mainImg, (group.mainPhoto.rotate || 'auto')));
          ...
          if (group.thumbnails && group.thumbnails.length > 0) {
            ...
            group.thumbnails.forEach(thumb => {
              ...
              thumbImg.addEventListener('load', () => applyRotation(thumbImg, thumb.rotate || 'auto'));
